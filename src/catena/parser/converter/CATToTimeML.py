...
class CATToTimeML:
        ...
        for (tid, tok, sid) in self.arr_tokens:
            if tok != None:
                ...
                if tid in self.event_starts:
                    ...
                    #print self.timex_starts[tid][0], self.timex_starts[tid][1], self.timex_starts[tid][2]
    ...
    def parseCAT(self):
        ...
        for event in events:
            start_id = event.findall("token_anchor")[0].get("t_id")
            end_id = event.findall("token_anchor")[-1].get("t_id")
            self.event_starts[start_id] = (event.get("m_id"), event.get("class"), event.get("pred"))
            ...
            if event.get("tense") != None and event.get("aspect") != None and event.get("polarity") != None and event.get("modality") != None and event.get("pos") != None:
                self.event_instances[event.get("m_id")] = (event.get("tense"), event.get("aspect"), event.get("polarity"), event.get("modality"), event.get("pos"))

        for timex in timexes:
            #if timex.get("TAG_DESCRIPTOR") == "Empty_Mark": #DCT
            if timex.get("TAG_DESCRIPTOR") == "DPT": #DCT
                self.dct = (timex.get("m_id"), timex.get("type"), timex.get("value"), timex.get("functionInDocument"))
            elif timex.get("TAG_DESCRIPTOR") == "Empty_Mark" or timex.get("TAG_DESCRIPTOR") == "Empty_Tag_timex3": #Empty timex
                pass
            else:         
                start_id = timex.findall("token_anchor")[0].get("t_id")
                end_id = timex.findall("token_anchor")[-1].get("t_id")
                self.timex_starts[start_id] = (timex.get("m_id"), timex.get("type"), timex.get("value"), timex.get("anchorTimeID"), timex.get("functionInDocument"))
                self.arr_timex_ends.append(end_id)

        for signal in signals:
            start_id = signal.findall("token_anchor")[0].get("t_id")
            end_id = signal.findall("token_anchor")[-1].get("t_id")
            self.signal_starts[start_id] = signal.get("m_id")
            self.arr_signal_ends.append(end_id)

        for csignal in csignals:
            start_id = csignal.findall("token_anchor")[0].get("t_id")
            end_id = csignal.findall("token_anchor")[-1].get("t_id")
            self.csignal_starts[start_id] = csignal.get("m_id")
            self.arr_csignal_ends.append(end_id)

        for tlink in tlinks:
            sid = tlink.find("source").get("m_id")
            tid = tlink.find("target").get("m_id")
            self.arr_tlinks.append((tlink.get("r_id"), tlink.get("relType"), tlink.get("signalID"), sid, tid))

        for slink in slinks:
            sid = slink.find("source").get("m_id")
            tid = slink.find("target").get("m_id")
            self.arr_slinks.append((slink.get("r_id"), slink.get("relType"), slink.get("signalID"), sid, tid))

        for alink in alinks:
            sid = alink.find("source").get("m_id")
            tid = alink.find("target").get("m_id")
            self.arr_alinks.append((alink.get("r_id"), alink.get("relType"), alink.get("signalID"), sid, tid))

        for clink in clinks:
            sid = clink.find("source").get("m_id")
            tid = clink.find("target").get("m_id")
            self.arr_clinks.append((clink.get("r_id"), clink.get("c-signalID"), sid, tid))

        for t in tokens:
            self.arr_tokens.append((t.get("t_id"), t.text, t.get("sentence")))
