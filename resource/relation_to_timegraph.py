...
def add_relation_in_timegraph(X, Y, rel, tg): 
...
##                nx2, tg = add_point_x_DURING_y_z(x2, x1, y2, tg)
    ...
    if handled == 'no':
        ...
        tg, match = interval_rel_X_Y(X, Y, tg, rel) 
        if match == 'false':
            tg.violated_relations += name+'\t'+X+'\t'+Y+'\t'+rel+'\t'+comment_n_weight+'\n'
            count_not_handled += 1 
            if debug >= 1: 
...
#    global entries_added 
    ...
    if not re.search(each, tg.entries_added): 
        ...
        match = add_relation_in_timegraph(words[1], words[2], words[3], tg)
...
def interval_rel_X_Y(X, Y, tg, corpus_rel):
    option = "evaluation"
